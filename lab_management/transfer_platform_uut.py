...
for uut in ut.objects.using('old').all():
    try:
        ...
        if uut.platformname:
            ...
            unit = u.objects.create(phase=phase,platform=platform,sn=uut.sn,sku=uut.sku,status=uut.unitstatus,scrap=scrap,position=position,keyin_time=uut.keyintime)
            unit.save()
